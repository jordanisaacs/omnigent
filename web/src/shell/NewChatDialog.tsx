...
export function NewChatLandingScreen() {
  ...
  useEffect(() => {
    const projectChanged = prefill.project !== prefillProject;
    const configChanged =
      !projectChanged &&
      prefillProject !== "" &&
  ...
  }, [prefillProject, prefill.project, prefillConfigSig]);
  ...
  useEffect(() => {
    ...
    seededConfigSigRef.current = prefillConfigSig;
  }, [prefill, prefillProject, prefillConfigSig]);
  ...
  const workspaceChip = (
    <button
      ...
      className="flex h-6 cursor-pointer items-center gap-1 rounded-full px-2.5 text-sm font-normal text-muted-foreground transition-colors hover:text-foreground disabled:cursor-default"
      ...
      <div className="flex w-full max-w-[840px] flex-col items-center gap-6 px-4 pt-8 pb-16 md:select-none md:px-10">
        <div className="flex w-full flex-col items-center justify-center gap-3.5">
          ...
          <h1 className="min-w-0 break-words text-center text-[1.5em] md:text-[2.15em] font-normal tracking-[-0.05em] text-foreground line-clamp-2 sm:text-left">
            {pmProject?.name || selectedProject || "What should we build?"}
          </h1>
        </div>
